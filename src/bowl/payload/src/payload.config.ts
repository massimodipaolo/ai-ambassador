...
import llm, { fineTuningJobsHandler, llmVectorDbsCleaningHandler, rulesHandler, toolsKnowledgeBaseHandler } from '@websolutespa/payload-plugin-bowl-llm';
    ...
    plugins: [
        ...
        collections: {
          ...
          [slug.llmVectorDb]: {
            adapter: USE_AZURE_ADAPTER ?
              azureBlobStorageAdapter({
                connectionString: process.env.AZURE_STORAGE_CONNECTION_STRING,
                containerName: process.env.AZURE_STORAGE_LLM_VECTORDB_CONTAINER_NAME,
                allowContainerCreate: process.env.AZURE_STORAGE_ALLOW_CONTAINER_CREATE === 'true',
                baseURL: process.env.AZURE_STORAGE_ACCOUNT_BASEURL,
              }) :
              fsStorageAdapter({
                baseDir: process.env.FS_STORAGE_LLM_VECTORDB_BASEDIR,
                baseURL: process.env.FS_STORAGE_LLM_VECTORDB_BASEURL,
              }),
            disablePayloadAccessControl: process.env.FS_STORAGE_DISABLE_PAYLOAD_ACCESS_CONTROL == 'true' ? true : undefined,
            generateFileURL: process.env.FS_STORAGE_ENABLE_GENERATE_FILE_URL == 'true' ? ({ filename }) => `${process.env.FS_STORAGE_LLM_VECTORDB_BASEURL}/${filename}` : undefined,
          },
        },
      }),
      seo({
      ...
      cronJob({
        ...
        jobs: {
          ...
          llmVectorDbFilesCleaning: {
            execute: async (payload: Payload) => {
              console.log('ScheduledTask.llmVectorDbFilesCleaning every day at 06:00');
              return await llmVectorDbsCleaningHandler(payload);
            },
            cron: '0 6 * * *',
          },
        },
